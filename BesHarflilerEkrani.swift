//
//  BesHarflilerEkrani.swift
//  kelimeOyunu
//
//  Created by ceyda oymak on 16.10.2024.
//

import UIKit

class BesHarflilerEkrani: UIViewController,UITextFieldDelegate {
    ...
    func loadWords() -> [String]{
        guard let fileURL=Bundle.main.url(forResource: "5harfliler", withExtension: "txt") else {
            return []
        }
        do {
    ...
    func getRandomWord(from words: [String])->String?{
        ...
        let randomFiveLetterWord=Int(arc4random_uniform(UInt32(words.count)))
        return words[randomFiveLetterWord]
    ...
    func isRowComplete() -> Bool {
        let startIndex = currentRowIndex*5
    ...
    func moveToNextRow() {
        ...
        let prevRowStartIndex = (currentRowIndex - 1) * 5
        if nextRowStartIndex < letterBoxes.count {
            ...
            for i in 0..<5 {
                ...
                if letterBoxes[prevIndex].backgroundColor == UIColor.green {
                    if i==0{
    ...
    func checkUserGuess() {
        ...
        if userGuesses.contains(userGuess) {
            for i in startIndex..<endIndex {
                ...
                letterBoxes[i].text = String(selectedword[selectedword.index(selectedword.startIndex, offsetBy: i % 5)])
            }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                self.performSegue(withIdentifier: "gameover2", sender: nil)
        ...
        if userGuess == selectedword {
            ...
            for i in startIndex..<endIndex{
                ...
                let correctLetter=String(selectedWordArray[i%5])
                
                if  userLetter == correctLetter{
                    letterBoxes[i].backgroundColor = UIColor.green
                    usedIndices.insert(i % 5)
                }
            }
